...
struct RVNGSVGDrawingGeneratorPrivate
{
	...
	// Text object state (for multi-line + alignment via tspans)
	double m_textX;          // frame left, inches
	double m_textY;          // frame top, inches
	double m_textWidth;      // frame width, inches
	double m_textHeight;     // frame height, inches
	std::string m_textAlign; // left/center/end/justify
	RVNGPropertyList m_currentSpanProps;
	bool m_inSpan;
	bool m_spanOpen;
	bool m_atLineStart;      // next tspan should set x (and y/dy)
	bool m_firstLine;        // true until first content line is placed
	double m_currentFontSizePt;
	double m_lineHeightFactor; // multiplier of font size (Corel/ODF line-height)
	double m_lineAdvancePt;  // dy for the next new line (from previous line's font)

	void writeSpanAttributes(const RVNGPropertyList &pList);
	void openTspan();
	void closeTspanIfOpen();
	void advanceLine();
	static double lineHeightFactorFromProps(const RVNGPropertyList &pList);
};
...
RVNGSVGDrawingGeneratorPrivate::RVNGSVGDrawingGeneratorPrivate(RVNGStringVector &vec, const RVNGString &nmSpace) :
	...
	m_table(),
	m_textX(0.0), m_textY(0.0), m_textWidth(0.0), m_textHeight(0.0),
	m_textAlign("left"), m_currentSpanProps(), m_inSpan(false), m_spanOpen(false),
	m_atLineStart(true), m_firstLine(true), m_currentFontSizePt(12.0),
	m_lineHeightFactor(1.0), m_lineAdvancePt(12.0)
{
	if (!m_nmSpace.empty())
		m_nmSpaceAndDelim = m_nmSpace+":";
}

double RVNGSVGDrawingGeneratorPrivate::lineHeightFactorFromProps(const RVNGPropertyList &pList)
{
	// Corel artistic text defaults near 100% of point size; CSS/ODF often use 120%.
	// Prefer an explicit fo:line-height when libcdr (or others) provide one.
	if (!pList["fo:line-height"])
		return 1.0;
	const RVNGString s = pList["fo:line-height"]->getStr();
	if (!s.cstr() || !s.cstr()[0])
		return 1.0;
	const char *c = s.cstr();
	char *end = nullptr;
	...
	if (end && *end == '%')
		return v / 100.0;
	// Unitless number: treat as multiplier (1.0) not points
	if (!end || !*end)
		return v;
	// Length with unit: approximate as multiplier of current font when possible
	// is left to caller; here return raw if no % (fallback 1.0 for safety if huge)
	if (v > 0.0 && v < 10.0)
		return v;
	return 1.0;
}
...
void RVNGSVGDrawingGeneratorPrivate::writeSpanAttributes(const RVNGPropertyList &pList)
{
	// Accept both style:font-name (ODF) and fo:font-family
	if (pList["style:font-name"])
		m_outputSink << "font-family=\"" << pList["style:font-name"]->getStr().cstr() << "\" ";
	else if (pList["fo:font-family"])
		m_outputSink << "font-family=\"" << pList["fo:font-family"]->getStr().cstr() << "\" ";
	if (pList["fo:font-style"])
		m_outputSink << "font-style=\"" << pList["fo:font-style"]->getStr().cstr() << "\" ";
	if (pList["fo:font-weight"])
		m_outputSink << "font-weight=\"" << pList["fo:font-weight"]->getStr().cstr() << "\" ";
	if (pList["fo:font-variant"])
		m_outputSink << "font-variant=\"" << pList["fo:font-variant"]->getStr().cstr() << "\" ";
	if (pList["fo:font-size"])
	{
		m_currentFontSizePt = pList["fo:font-size"]->getDouble();
		m_outputSink << "font-size=\"" << doubleToString(m_currentFontSizePt) << "\" ";
	}
	if (pList["fo:color"])
		m_outputSink << "fill=\"" << pList["fo:color"]->getStr().cstr() << "\" ";
	if (pList["fo:text-transform"])
		m_outputSink << "text-transform=\"" << pList["fo:text-transform"]->getStr().cstr() << "\" ";
	if (pList["svg:fill-opacity"])
		m_outputSink << "fill-opacity=\"" << doubleToString(pList["svg:fill-opacity"]->getDouble()) << "\" ";
	if (pList["svg:stroke-opacity"])
		m_outputSink << "stroke-opacity=\"" << doubleToString(pList["svg:stroke-opacity"]->getDouble()) << "\" ";
}

void RVNGSVGDrawingGeneratorPrivate::closeTspanIfOpen()
{
	if (m_spanOpen)
	{
		m_outputSink << "</" << getNamespaceAndDelim() << "tspan>\n";
		m_spanOpen = false;
	}
}

void RVNGSVGDrawingGeneratorPrivate::advanceLine()
{
	// Remember advance from the font size of the line we are leaving
	m_lineAdvancePt = m_currentFontSizePt * m_lineHeightFactor;
	m_atLineStart = true;
	closeTspanIfOpen();
}

void RVNGSVGDrawingGeneratorPrivate::openTspan()
{
	m_outputSink << "<" << getNamespaceAndDelim() << "tspan ";
	if (m_atLineStart)
	{
		...
		{
			x = m_textX + m_textWidth / 2.0;
			anchor = "middle";
		}
		else if (m_textAlign == "end" || m_textAlign == "right")
		{
			x = m_textX + m_textWidth;
			anchor = "end";
		}

		m_outputSink << "x=\"" << doubleToString(72.0 * x) << "\" ";
		if (m_firstLine)
		{
			// First baseline near top of frame. Use ~0.85em (approx. Latin
			// capital height / ascender), not a full em — matching Corel's
			// ink-box layout leaves more of the frame for inter-line pitch.
			const double firstLineEm = 0.85;
			const double baselineY = m_textY + (m_currentFontSizePt * firstLineEm) / 72.0;
			m_outputSink << "y=\"" << doubleToString(72.0 * baselineY) << "\" ";
			m_firstLine = false;
		}
		else
		{
			// Relative advance using previous line's font metrics (user units == pt here)
			m_outputSink << "dy=\"" << doubleToString(m_lineAdvancePt) << "\" ";
		}
		m_outputSink << "text-anchor=\"" << anchor << "\" ";
		m_atLineStart = false;
		// Next soft break should advance using this line's font size + spacing
		m_lineHeightFactor = lineHeightFactorFromProps(m_currentSpanProps);
		m_lineAdvancePt = m_currentFontSizePt * m_lineHeightFactor;
	}
	// Continuation spans on the same line omit x/y so glyphs flow horizontally
	writeSpanAttributes(m_currentSpanProps);
	m_outputSink << ">\n";
	m_spanOpen = true;
}
...
void RVNGSVGDrawingGenerator::startTextObject(const RVNGPropertyList &propList)
{
	...
	if (propList["svg:width"])
	{
		width = getInchValue(*propList["svg:width"]);
		xmiddle += width / 2.0;
	}
	...
	if (propList["fo:padding-top"])
		y += propList["fo:padding-top"]->getDouble();

	// Keep frame top-left; per-line baselines are computed in openTspan()
	m_pImpl->m_textX = x;
	m_pImpl->m_textY = y;
	m_pImpl->m_textWidth = width;
	m_pImpl->m_textHeight = height;
	m_pImpl->m_textAlign = "left";
	m_pImpl->m_inSpan = false;
	m_pImpl->m_spanOpen = false;
	m_pImpl->m_atLineStart = true;
	m_pImpl->m_firstLine = true;
	m_pImpl->m_currentFontSizePt = 12.0;
	m_pImpl->m_lineHeightFactor = 1.0;
	m_pImpl->m_lineAdvancePt = 12.0;
	m_pImpl->m_currentSpanProps = RVNGPropertyList();

	// Parent text position is a default; line tspans set absolute x/y
	m_pImpl->m_outputSink << "x=\"" << doubleToString(72*x) << "\" y=\"" << doubleToString(72*y) << "\"";
...
void RVNGSVGDrawingGenerator::openParagraph(const RVNGPropertyList &propList)
{
	if (propList["fo:text-align"])
		m_pImpl->m_textAlign = propList["fo:text-align"]->getStr().cstr();
	else
		m_pImpl->m_textAlign = "left";
}
...
{
	// Next paragraph begins on the following visual line (if any content follows)
	m_pImpl->advanceLine();
}
...
void RVNGSVGDrawingGenerator::openSpan(const RVNGPropertyList &propList)
{
	...
	// Update font metrics; defer opening a tspan until there is real text
	// so empty spans that only contain line breaks do not create blank lines.
	if (pList["fo:font-size"])
		m_pImpl->m_currentFontSizePt = pList["fo:font-size"]->getDouble();
	m_pImpl->m_lineHeightFactor = RVNGSVGDrawingGeneratorPrivate::lineHeightFactorFromProps(pList);
	m_pImpl->m_lineAdvancePt = m_pImpl->m_currentFontSizePt * m_pImpl->m_lineHeightFactor;

	m_pImpl->m_currentSpanProps = pList;
	m_pImpl->m_inSpan = true;
	m_pImpl->closeTspanIfOpen();
}
...
void RVNGSVGDrawingGenerator::insertText(const RVNGString &str)
{
	if (str.empty())
		return;
	if (!m_pImpl->m_spanOpen && m_pImpl->m_inSpan)
		m_pImpl->openTspan();
	else if (!m_pImpl->m_spanOpen)
	{
		// insertText outside a span should still render
		m_pImpl->openTspan();
	}
...
{
	// Soft line break: do not open an empty tspan here. The next insertText/openSpan
	// with content will open a positioned tspan on the following baseline.
	m_pImpl->advanceLine();
}
