...
struct CDRStyle
{
  ...
  double m_fontSize;
  // CSS/ODF font-weight (100-900). 0 = not set.
  unsigned m_fontWeight;
  // -1 = not set, 0 = normal, 1 = italic
  signed char m_fontItalic;
  // Line height as multiplier of font size (Corel %/100). 0 = not set.
  ...
  CDRStyle()
    : m_charSet((unsigned short)-1), m_fontName(),
      m_fontSize(0.0), m_fontWeight(0), m_fontItalic(-1), m_lineSpacing(0.0),
      m_align(0), m_leftIndent(0.0), m_firstIndent(0.0),
  ...
  void overrideStyle(const CDRStyle &override)
  {
    ...
    if (override.m_fontWeight)
      m_fontWeight = override.m_fontWeight;
    if (override.m_fontItalic >= 0)
      m_fontItalic = override.m_fontItalic;
    if (!CDR_ALMOST_ZERO(override.m_lineSpacing))
      m_lineSpacing = override.m_lineSpacing;
    if (override.m_align)
  ...
  // Map Corel character style-flag bits (txsm fl2&2 / fontTable) to weight/italic.
  // Observed bits: 0x40 Regular, 0x100 Medium, 0x400 SemiBold, 0x1000 Bold,
  // 0x4000 ExtraBold, 0x20000 Black Italic.
  void applyFontStyleFlags(unsigned flags)
  {
    if (!flags)
      return;
    // Prefer heaviest matching weight bit.
    if (flags & 0x20000u)
    {
      m_fontWeight = 900;
      m_fontItalic = 1;
      ...
      m_fontWeight = 400;
    else if (flags & 0x1u) // classic bold (legacy)
      m_fontWeight = 700;

    // Classic italic bit when not already set via a named italic face bit.
    if (m_fontItalic < 0 && (flags & 0x2u))
      m_fontItalic = 1;
    else if (m_fontItalic < 0 && m_fontWeight)
      m_fontItalic = 0;
  }
};
