...
void libcdr::CDRParser::readFild(librevenge::RVNGInputStream *input, unsigned length)
{
  ...
  if (m_version >= 1500)
  {
    // X5+ fild: u32 unknown, u32 hdrLen, hdrLen bytes (typically a GUID),
    // then optional X3-style tags (0x514/0x640), then fillType.
    // Older code only skipped 8 bytes and mis-read fillType from the GUID,
    // so every solid fill became "none"/wrong.
    input->seek(4, librevenge::RVNG_SEEK_CUR);
    unsigned hdrLen = readU32(input);
    if (hdrLen > getRemainingLength(input))
      throw GenericException();
    input->seek(hdrLen, librevenge::RVNG_SEEK_CUR);
    _skipX3Optional(input);
  }
  else if (m_version >= 1300)
    input->seek(8, librevenge::RVNG_SEEK_CUR);
  unsigned short fillType = readU16(input);
  CDR_DEBUG_MSG(("CDRParser::readFild fillId=0x%x fillType=%u version=%u\n", fillId, fillType, m_version));
  libcdr::CDRColor color1;
  libcdr::CDRColor color2;
  libcdr::CDRImageFill imageFill;
  libcdr::CDRGradient gradient;
  ...
  CDR_DEBUG_MSG(("CDRParser::readFild done fillId=0x%x fillType=%u color1 model=0x%x value=0x%.8x\n",
                 fillId, fillType, color1.m_colorModel, color1.m_colorValue));
...
void libcdr::CDRParser::readBmp(librevenge::RVNGInputStream *input, unsigned length)
{
  ...
  // X5+ bitmap containers often append an 8-bit alpha/mask plane after the
  // RGB RImage, preceded by a short header (commonly 78 bytes).
  std::vector<unsigned char> alpha;
  if (width && height && length)
  {
    long used = input->tell() - chunkStart;
    long trail = (long)length - used;
    unsigned long planeSize = (unsigned long)width * (unsigned long)height;
    if (trail > 0 && (unsigned long)trail >= planeSize)
    {
      unsigned long hdr = (unsigned long)trail - planeSize;
      if (hdr <= 256)
      {
        if (hdr)
          input->seek((long)hdr, librevenge::RVNG_SEEK_CUR);
        unsigned long nread = 0;
        const unsigned char *abuf = input->read(planeSize, nread);
        if (nread == planeSize)
          alpha.assign(abuf, abuf + nread);
        CDR_DEBUG_MSG(("CDRParser::readBmp id=%u alpha plane hdr=%lu size=%lu\n",
                       imageId, hdr, nread));
      }
    }
  }

  m_collector->collectBmp(imageId, colorModel, width, height, bpp, palette, bitmap, alpha);
}
...
void libcdr::CDRParser::readStlt(librevenge::RVNGInputStream *input, unsigned length)
{
  ...
  try
  {
    ...
    std::map<unsigned,unsigned> fontStyleFlags;
    for (unsigned long i=0; i<numFonts; ++i)
    {
      ...
      if (m_version < 1000)
      {
        // Pre-X4: 12-byte trailer; style flags at offset 0 (best effort)
        unsigned styleFlags = readU32(input);
        fontStyleFlags[fontStyleId] = styleFlags;
        input->seek(8, librevenge::RVNG_SEEK_CUR);
      }
      else
      {
        // Two coordinates then the multi-weight face flags dword
        input->seek(8, librevenge::RVNG_SEEK_CUR);
        unsigned styleFlags = readU32(input);
        fontStyleFlags[fontStyleId] = styleFlags;
        input->seek(8, librevenge::RVNG_SEEK_CUR);
        CDR_DEBUG_MSG(("CDRParser::readStlt fontStyleId=%u styleFlags=0x%x\n",
                       fontStyleId, styleFlags));
      }
    }
    ...
    // Each interval record is 52 bytes: id + 12×u32. Corel stores spacing
    // factors as millionths (1000000 = 100% of font size). Layout:
    // [0..3] flags/padding, [4] char spacing, [5] line spacing, [6..].
    std::map<unsigned, double> lineSpacings;
    for (unsigned ii=0; ii<numIntervals; ++ii)
    {
      unsigned id = readU32(input);
      input->seek(16, librevenge::RVNG_SEEK_CUR); // 4 unused dwords
      unsigned charSpacing = readU32(input);
      unsigned lineSpacing = readU32(input);
      input->seek(24, librevenge::RVNG_SEEK_CUR); // remaining 6 dwords
      (void)charSpacing;
      if (lineSpacing)
        lineSpacings[id] = (double)lineSpacing / 1000000.0;
      CDR_DEBUG_MSG(("CDRParser::readStlt interval %u lineSpacing=%u (%.3f)\n",
                     id, lineSpacing, lineSpacings.count(id) ? lineSpacings[id] : 0.0));
    }
    ...
    try
    {
           ...
           iter != styles.end(); ++iter)
      {
        ...
        if (fontRecId)
        {
          ...
          std::map<unsigned, unsigned>::const_iterator iterFontFlags = fontStyleFlags.find(fontRecId);
          if (iterFontFlags != fontStyleFlags.end())
            tmpCharStyle.applyFontStyleFlags(iterFontFlags->second);
        }
        if (iter->second.intervalId)
        {
          std::map<unsigned, double>::const_iterator iterSp = lineSpacings.find(iter->second.intervalId);
          if (iterSp != lineSpacings.end())
            tmpCharStyle.m_lineSpacing = iterSp->second;
        }
  ...
  try
  {
    ...
    for (unsigned j = 0; j < numPara; ++j)
    {
      ...
      for (unsigned i = 0; i < numStyles; ++i)
      {
        ...
        if (fl2&2) // Bold/Italic / multi-weight face flags
        {
          unsigned styleFlags = readU32(input);
          style.applyFontStyleFlags(styleFlags);
          CDR_DEBUG_MSG(("CDRParser::readTxsm styleFlags=0x%x weight=%u italic=%d font=%s\n",
                         styleFlags, style.m_fontWeight, (int)style.m_fontItalic,
                         style.m_fontName.cstr() ? style.m_fontName.cstr() : "(none)"));
        }
        if (fl2&4) // Font Size
  ...
  for (unsigned j=0; j<numPara; ++j)
  {
    ...
    for (unsigned i=0; i<numSt; ++i)
    {
      ...
      if (flag&0x01)
      {
        ...
        input->seek(4, librevenge::RVNG_SEEK_CUR);
      // Fixed layout: style-flag dword (bold/italic/weight face bits)
      style.applyFontStyleFlags(readU32(input));
      if (flag&0x04)
        style.m_fontSize = readCoordinate(input);
      else
        input->seek(4, librevenge::RVNG_SEEK_CUR);
