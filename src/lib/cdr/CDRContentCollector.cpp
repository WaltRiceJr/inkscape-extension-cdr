...
void libcdr::CDRContentCollector::_flushCurrentPath()
{
  ...
  CDROutputElementList outputElement;
  // Bitmap loda records also carry a frame path. Painting that path with the
  // object fill (often solid black) puts an opaque plate under the image and
  // defeats PNG/alpha transparency. Emit only the graphic when we have a
  // bitmap; the path is not used as a clip today.
  const bool hasBitmap = m_currentImage.getImage().size() != 0;
  if (!hasBitmap && (!m_currentPath.empty() || (!m_splineData.empty() && m_isInSpline)))
  {
  ...
  {
    // Discard frame path so it does not leak into a later object.
    m_currentPath.clear();
    m_polygon.reset();
    m_isInPolygon = false;
    m_splineData.clear();
  ...
  if (m_currentImage.getImage().size())
  {
    ...
    {
      const librevenge::RVNGBinaryData &img = m_currentImage.getImage();
      const unsigned char *buf = img.getDataBuffer();
      bool isPng = (img.size() >= 8 && buf &&
                    buf[0]==0x89 && buf[1]==0x50 && buf[2]==0x4e && buf[3]==0x47 &&
                    buf[4]==0x0d && buf[5]==0x0a && buf[6]==0x1a && buf[7]==0x0a);
      propList.insert("librevenge:mime-type", isPng ? "image/png" : "image/bmp");
      propList.insert("office:binary-data", img);
    }
  ...
  if (m_currentText && !m_currentText->empty())
  {
    ...
    const double frameHeightIn = fabs(y2 - y1);
    const double frameHeightPt = frameHeightIn * 72.0;

    // Resolve line-height for this text object.
    // Corel stores interLineSpacing as millionths of "char height" (1000000=100%).
    // Using that factor alone as SVG baseline dy is too tight: Corel's % is relative
    // to character height / single spacing, while our generator advances baselines by
    // factor×font-size. Scale so 100% ≈ single spacing at ~1.0em, and when the text
    // frame is a tight fit around multi-line content, prefer the frame-implied pitch
    // (matches how the box was sized in Corel).
    double styleLineFactor = 0.0;
    double maxFontSizePt = 0.0;
    unsigned softBreaks = 0;
    for (const auto &line : *m_currentText)
    {
      for (const auto &span : line.m_line)
      {
        if (!CDR_ALMOST_ZERO(span.m_style.m_lineSpacing) &&
            span.m_style.m_lineSpacing > styleLineFactor)
          styleLineFactor = span.m_style.m_lineSpacing;
        double fs = (double)cdr_round(144.0 * span.m_style.m_fontSize) / 2.0;
        if (fs > maxFontSizePt)
          maxFontSizePt = fs;
        if (!span.m_text.empty())
        {
          const char *p = span.m_text.cstr();
          if (p)
          {
            for (; *p; ++p)
              if (*p == '\n')
    ...
    if (CDR_ALMOST_ZERO(styleLineFactor))
      styleLineFactor = 1.0; // Corel default interLineSpacing

    // Effective multiplier for baseline advance (1.0 = one em).
    // Keep in sync with RVNGSVGDrawingGenerator first baseline (~0.85em).
    // Using a full em for the first line under-assigns the frame height to
    // inter-line pitch and reads too tight next to Corel.
    const double firstLineEm = 0.85;
    double lineHeightFactor = styleLineFactor;
    const unsigned nLines = softBreaks + 1;
    if (nLines >= 2 && maxFontSizePt > 1.0 &&
        frameHeightPt > maxFontSizePt * firstLineEm)
    {
      const double fromFrame =
        (frameHeightPt - maxFontSizePt * firstLineEm) /
        ((double)(nLines - 1) * maxFontSizePt);
      if (fromFrame >= 0.55 && fromFrame <= 1.45)
      {
        // Prefer frame pitch when it is not far above the style value
        // (avoids stretching into empty paragraph boxes).
        if (fromFrame <= styleLineFactor * 1.4 + 0.15)
          lineHeightFactor = fromFrame;
        else
          lineHeightFactor = styleLineFactor < 1.0 ? 1.0 : styleLineFactor;
      }
    }

    librevenge::RVNGPropertyList textFrameProps;
    textFrameProps.insert("svg:width", fabs(x2-x1));
    textFrameProps.insert("svg:height", frameHeightIn);
    ...
    for (const auto &i : *m_currentText)
    {
      ...
      for (const auto &j : currentLine)
      {
        if (!j.m_text.empty())
        {
          ...
          {
            // ODF name (LibreOffice) and CSS family (Inkscape via SVG generator)
            spanProps.insert("style:font-name", j.m_style.m_fontName);
            spanProps.insert("fo:font-family", j.m_style.m_fontName);
          }
          if (j.m_style.m_fontWeight)
          {
            // Numeric CSS/ODF weights so multi-weight families (e.g. Aleo) resolve.
            librevenge::RVNGString weightStr;
            weightStr.sprintf("%u", j.m_style.m_fontWeight);
            spanProps.insert("fo:font-weight", weightStr);
          }
          if (j.m_style.m_fontItalic >= 0)
            spanProps.insert("fo:font-style", j.m_style.m_fontItalic ? "italic" : "normal");
          if (!CDR_ALMOST_ZERO(lineHeightFactor))
          {
            librevenge::RVNGString lh;
            lh.sprintf("%.4g%%", lineHeightFactor * 100.0);
            spanProps.insert("fo:line-height", lh);
          }
...
void libcdr::CDRContentCollector::_generateBitmapFromPattern(librevenge::RVNGBinaryData &bitmap, const CDRPattern &pattern, const CDRColor &fgColor, const CDRColor &bgColor)
{
  ...
  // Force opaque alpha: BI_RGB 32bpp with A=0 is treated as transparent/black by many hosts.
  unsigned foreground = m_ps._getRGBColor(fgColor) | 0xff000000u;
  unsigned background = m_ps._getRGBColor(bgColor) | 0xff000000u;
